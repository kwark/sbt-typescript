...
(function (ts) {
    ts.Diagnostics = {
        ...
        Watch_input_files: { code: 6005, category: 2 /* Message */, key: "s input files." },
    ...
    function emitFiles(resolver, targetSourceFile) {
        ...
        function emitJavaScript(jsFilePath, root) {
            ...
            function emitExpressionIdentifier(node) {
                var symbol = resolver.getNodeLinks(node).resolvedSymbol;
                if (symbol) {
            ...
            function emitMethodSuperClass(node, isConstructor) {
                if (currentClass) {
                    write(getDeclClassFullName(currentClass));
                    write('.superclass.');
                    write(isConstructor ? 'constructor' : node.parent.name.text);
                }
                else {
            ...
            function emitCallExpression(node) {
                var superCall = false;
                if (node.expression.kind === 89 /* SuperKeyword */) {
                    emitMethodSuperClass(node, true);
                    superCall = true;
                }
                else {
                    emit(node.expression);
            ...
            function getModuleFullName(container) {
                var parts = [];
                while (container && container.kind == 189 /* ModuleDeclaration */) {
            ...
            function emitModuleMemberName(node) {
                ...
                if (node.flags & 1 /* Export */) {
                    ...
                    if (container && node.kind == 183 /* VariableDeclaration */) {
                        write("Ext.ns('");
            ...
            function isClassMemberMethod(node) {
                return (node.kind === 126 /* Constructor */ || node.kind === 125 /* Method */ || node.kind === 127 /* GetAccessor */ || node.kind === 128 /* SetAccessor */);
            }
            ...
            function emitClassMember(member, emitConstructorOfClass, toemit, emitted) {
            ...
            function emitClassMembers(node, emitConstructorOfClass) {
                var totalStatic = 0;
                var totalInstance = 0;
                var emitted = 0;
                var baseTypeNode = ts.getClassBaseTypeNode(node);
                ts.forEach(node.members, function (member) {
                    ...
                    totalStatic += ((member.flags & 128 /* Static */) && emit) ? 1 : 0;
                    totalInstance += (!(member.flags & 128 /* Static */) && emit) ? 1 : 0;
                });
                if (baseTypeNode) {
                    writeLine();
                ...
                ts.forEach(node.members, function (member) {
                    var lemitted = false;
                    if (!(member.flags & 128 /* Static */)) {
                        if (member.kind === 126 /* Constructor */) {
                            if (member.body) {
                                writeLine();
            ...
            function emitSourceFile(node) {
                ...
                if (false && !extendsEmitted && resolver.getNodeCheckFlags(node) & 8 /* EmitExtends */) {
                    writeLine();
